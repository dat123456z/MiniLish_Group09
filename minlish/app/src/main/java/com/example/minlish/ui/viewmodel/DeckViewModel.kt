...
class DeckViewModel(application: Application) : AndroidViewModel(application) {
    ...
    fun loadDecks(ownerId: String) {
        viewModelScope.launch {
            ...
            for (deck in decks) {
                ...
                val vocabIdsSet = vocabs.map { it.id }.toSet()
                val srsCards = srsRepo.getByDeck(deck.id)
                val learnedCount = srsCards.count { it.repetitions > 0 && it.vocabId in vocabIdsSet }
    ...
    fun addDeck(name: String, description: String, tags: String, ownerId: String, newWords: Int = 10, reviews: Int = 40) {
        if (name.isBlank()) { errorState.value = "Deck name cannot be empty"; return }
        viewModelScope.launch {
            val deck = Deck(
                name = name,
    ...
    fun addVocab(
        ...
        viewModelScope.launch {
            ...
            val vocabIdsSet = updatedVocabs.map { it.id }.toSet()
    ...
    fun updateVocab(
        vocabId: String, word: String, pronunciation: String, meaning: String,
        description: String, example: String, collocation: String,
        relatedWords: String, note: String, deckId: String
    ) {
        viewModelScope.launch {
    ...
    fun importCsv(uri: Uri, deckId: String) {
        viewModelScope.launch {
            ...
            if (result.error != null) {
                importResultState.postValue("Error: ${result.error}")
            } else {
                importResultState.postValue("Imported ${result.imported} words, skipped ${result.skipped}")
                vocabListState.postValue(vocabRepo.getByDeck(deckId))
            }
            isLoadingState.postValue(false)
        }
    ...
    fun importCsvWithNewDeck(uri: Uri, deckName: String, deckDescription: String, ownerId: String) {
        if (deckName.isBlank()) { errorState.value = "Deck name cannot be empty"; return }
        viewModelScope.launch {
            isLoadingState.postValue(true)
            val deck = Deck(
                name = deckName,
            ...
            if (result.error != null) {
                ...
                importResultState.postValue("Created deck \"${deckName}\" and imported ${result.imported} words, skipped ${result.skipped}")
                loadDecks(ownerId)
            }
            isLoadingState.postValue(false)
        }
    ...
    fun exportCsv(uri: Uri, deckId: String) {
        viewModelScope.launch {
            ...
            importResultState.postValue(if (success) "Export successful" else "Export failed")
        }
    }
